...
enum Strategy {
    RoundRobin {
        next_index_for_project: Vec<usize>,
    },
    WeightedRoundRobin {
        usage_counts_for_project: Vec<Vec<usize>>,
    },
    Balancer {},
}
...
struct SimulationHyperParameters {
    ...
    static_project_duplication_count: usize,
    strategy: Strategy,
    intensity: f32,
}
...
fn simulate_fake(
        ...
        SimulationHyperParameters{ input_intensity, request_amount, mut users, mut strategy,
            project_names: _, dbs, synchronize_db_changes: _, static_project_duplication_count, .. }: SimulationHyperParameters) -> SimulationOutput {
    ...
    'simulation_loop: loop {
        ...
        if iteration == spawn_wake_at && !spawning_done {
            'spawning_loop: loop { // loop is used to make instant multi-spawning possible
                ...
                if !library.project_registered(request.project_id) {
                    match &mut strategy {
                        Strategy::RoundRobin { next_index_for_project } => {
                            for i in 0..static_project_duplication_count {
                                while next_index_for_project.len() <= project_id { next_index_for_project.push(0); }
                                if i == 0 {
                                    ...
                                    let db_id = unclaimed_worker_with_least_worktime(&requests_count_of_worker, &dbs, &able_worker_ids)
                                        .expect("nowhere to duplicate for RR");
                                    library.spread_project_to(project_id, db_id);
                                    // println!("Also assigning project {} to {}", request.project_id, db_id);
                                }
                            }
                        },
                        Strategy::WeightedRoundRobin { usage_counts_for_project } => {
                            for i in 0..static_project_duplication_count {
                                while usage_counts_for_project.len() <= project_id {
                                    usage_counts_for_project.push(vec![0; static_project_duplication_count]);
                                }
                                if i == 0 {
                                    let db_id = worker_with_least_worktime(&requests_count_of_worker, &dbs);
                                    library.register_new_project(request.project_id, db_id);
        ...
        // ======================== Put for execution =========================
        let task = match &mut strategy {
            Strategy::RoundRobin { next_index_for_project } => {
                // For each UserRequest...
                queue.iter() // NOTE: to make this a simple queue without look-through just .take(1)
                    // ... get its current db id
                    .map(|r| {
                        let index = next_index_for_project[r.project_id];
                        &library.dbs_for_project[&r.project_id][index]
                    })
                    .map(|selected_worker_id| if workers[*selected_worker_id].is_none() { Some(selected_worker_id) } else { None })
                    .enumerate()
            ...
            Strategy::WeightedRoundRobin { usage_counts_for_project } => {
                ...
                queue.iter() // NOTE: to make this a simple queue without look-through just .take(1)
                    // ... get its current db id
                    .map(|r| {
                        let my_dbs = &library.dbs_for_project[&r.project_id];
                        let (index, _) = usage_counts_for_project[r.project_id].iter().zip(my_dbs.iter())
                            .map(|(counts, db)| *counts as Time * dbs[*db].ping_millis)
                            .enumerate()
                            .min_by(|(_, r1), (_, r2)| {
                                // println!("Comparing {} with {}", r1, r2);
                                if r1 > r2 { std::cmp::Ordering::Greater } else { std::cmp::Ordering::Less }
                            }).unwrap();
                        my_dbs[index]
                    })
                    .map(|selected_worker_id| if workers[selected_worker_id].is_none() { Some(selected_worker_id) } else { None })
        ...
        if let Some((user_request_i, chosen_worker)) = task {
            ...
            match &mut strategy {
                Strategy::RoundRobin { next_index_for_project } => {
                    let index = &mut next_index_for_project[project_id];
                    *index += 1;
                    if *index >= static_project_duplication_count { *index = 0; }
                },
                Strategy::WeightedRoundRobin { usage_counts_for_project } => {
                    // Copy-pasted from above search for WRR:
                    let my_dbs = &library.dbs_for_project[&project_id];
                    let (index, _) = usage_counts_for_project[project_id].iter().zip(my_dbs.iter())
                        .map(|(counts, db)| *counts as Time * dbs[*db].ping_millis)
                        .enumerate()
                        .min_by(|(_, r1), (_, r2)| if r1 > r2 { std::cmp::Ordering::Greater } else { std::cmp::Ordering::Less }).unwrap();
                    let count = &mut usage_counts_for_project[project_id][index];
                    *count += 1;
                },
                Strategy::Balancer {} => {
                    // TODO: move stat into this Strategy
...
fn interpret_output(simulation_output: SimulationOutput) -> (Time, usize) {
    let mut average_waiting_time = 0;
    for UserRequest{ created_at, assigned_at, ..} in simulation_output.processed_user_requests.iter() {
        let assigned_at =         assigned_at        .expect("empty Option while describing processed request");
        let waiting_time = assigned_at.duration_since(&created_at).as_millis();
        average_waiting_time += waiting_time;
    }
    average_waiting_time /= simulation_output.processed_user_requests.len() as u128;
    let db_len = simulation_output.average_db_ping_millis.len();
    let projects_for_db = (0..db_len).into_iter()
        .map(|i| {
            let mut res = Vec::new();
            for (project, dbs) in simulation_output.dbs_for_project.iter() {
                if dbs.contains(&i) {
                    res.push(project);
                }
            }
    ...
    (average_waiting_time, projects_for_db.iter().map(|v| v.iter().count()).sum::<usize>())
}
...
async fn get_hyperparameters(random: &mut ChaChaRng, is_real: bool, strategy: Strategy, static_project_duplication_count: usize) -> MongoResult<SimulationHyperParameters> {
    ...
    Ok(SimulationHyperParameters {
        request_amount: 32 * 512,
        // input_intensity: None,
        input_intensity: Some(intensity * max_processing_intensity),
        // input_intensity: Some(1.05 * processing_intensity),
        dbs,
        project_names,
        users,
        synchronize_db_changes: false,
        static_project_duplication_count,
        strategy,
        intensity,
        // strategy: Strategy::Balancer{},
        // strategy: Strategy::RoundRobin{ next_index_for_project: vec![] },
        // strategy: Strategy::WeightedRoundRobin{ usage_counts_for_project: vec![] },
    })
}
// ============================================================================
// ============================================================================
...
async fn main() -> MongoResult<()> {
    let seed = 433;
    {
        ...
        let hyperparameters = get_hyperparameters(&mut random, false, Strategy::Balancer{}, 0).await?;
        let int = hyperparameters.intensity;
        let parameters = get_parameters();
        let output = simulate_fake(random, parameters, hyperparameters);
        describe_simulation_output(&output);
        let (avg, usage) = interpret_output(output);
        println!("Balancer: intensity={}, avg waiting time={}, usage={}", int, avg, usage);
    }
    {
        let mut random = ChaChaRng::seed_from_u64(seed);
        let hyperparameters = get_hyperparameters(&mut random, false, Strategy::RoundRobin{ next_index_for_project: vec![] }, 2).await?;
        let int = hyperparameters.intensity;
        let parameters = get_parameters();
        let output = simulate_fake(random, parameters, hyperparameters);
        let (avg, usage) = interpret_output(output);
        println!("RR-2    : intensity={}, avg waiting time={}, usage={}", int, avg, usage);
    }
    {
        let mut random = ChaChaRng::seed_from_u64(seed);
        let hyperparameters = get_hyperparameters(&mut random, false, Strategy::WeightedRoundRobin{ usage_counts_for_project: vec![] }, 2).await?;
        let int = hyperparameters.intensity;
        let parameters = get_parameters();
        let output = simulate_fake(random, parameters, hyperparameters);
        let (avg, usage) = interpret_output(output);
        println!("WRR-2   : intensity={}, avg waiting time={}, usage={}", int, avg, usage);
    }
    {
        let mut random = ChaChaRng::seed_from_u64(seed);
        let hyperparameters = get_hyperparameters(&mut random, false, Strategy::RoundRobin{ next_index_for_project: vec![] }, 3).await?;
        let int = hyperparameters.intensity;
        let parameters = get_parameters();
        let output = simulate_fake(random, parameters, hyperparameters);
        let (avg, usage) = interpret_output(output);
        println!("RR-3    : intensity={}, avg waiting time={}, usage={}", int, avg, usage);
    }
    {
        let mut random = ChaChaRng::seed_from_u64(seed);
        let hyperparameters = get_hyperparameters(&mut random, false, Strategy::WeightedRoundRobin{ usage_counts_for_project: vec![] }, 3).await?;
        let int = hyperparameters.intensity;
        let parameters = get_parameters();
        let output = simulate_fake(random, parameters, hyperparameters);
        let (avg, usage) = interpret_output(output);
        println!("WRR-3   : intensity={}, avg waiting time={}, usage={}", int, avg, usage);
    }

    // let mut random = ChaChaRng::seed_from_u64(433);
    //
